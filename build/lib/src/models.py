...
class RecPCN(nn.Module):
    ...
    def energy_exp(self, X):
        """Calculate the energy value of the implicit model given a batch of input
        Aka the squared error

        Returns a vector of energies, indicating the energy value for each input in X
        """
        return torch.sum(torch.exp(X - self.forward(X)),dim=-1)
...
class MultilayerPCN(nn.Module):
    ...
    def test_pc_generative(self, corrupt_inp, n_iters, update_mask, sensory=True):
        ...
        for itr in range(n_iters):
            self.update_val_nodes(update_mask, recon=False) # no need to relax sensory layer for ND/RM so I changed recon to False - Ed
    ...
    def update_grads(self, penalty):
        ...
        for l in range(self.n_layers-1):
            ...
            if l == self.n_layers - 2:
                grad_w += penalty * torch.sign(self.layers[l].weight)
            self.layers[l].weight.grad = grad_w
            if self.use_bias:
                self.layers[l].bias.grad = -torch.sum(self.errs[l+1], axis=0)
    
class ConvolutionalPCN(MultilayerPCN):
    def __init__(self, nodes, nonlin, Dt, kernel_sizes, strides, paddings, init_std=0., lamb=0, use_bias=False):
        super().__init__(nodes, nonlin, Dt, lamb, use_bias)
        self.memory = nn.Parameter(init_std * torch.randn((self.mem_dim,)))
        # Set the weights initialized to 0 for weights outside the receptive field
        self.masks = []
        ...
        for l in range(self.n_layers-1):
            ...
            if strides[l] == 0:
                ...
                self.masks.append(torch.ones(nodes[l+1], nodes[l]))
                print("self.masks l = 0' shape:", self.masks[l].shape)
            else:
                self.masks.append(utils.create_all_masks(round(nodes[l+1]**0.5), kernel_sizes[l], strides[l], paddings[l]))
            # self.layers[l].weight = self.layers[l].weight * self.masks[l]
            print("Weight shape:", self.layers[l].weight.shape)
            print("Mask shape:", self.masks[l].shape)
            # self.layers[l].weight.to(self.device)
            print("weights are on device:", self.layers[l].weight.device)
            self.layers[l].weight = torch.nn.Parameter(self.layers[l].weight * self.masks[l])
    ...
    def update_grads(self,penalty):
        ...
        for l in range(self.n_layers-1):
            grad_w = -torch.matmul(self.errs[l+1].t(), self.nonlins[l](self.val_nodes[l]))
            if grad_w.shape != self.masks[l].shape:
                raise ValueError("grad_w shape does not match mask shape")
            grad_w = grad_w * self.masks[l].to(self.device)
            # below not needed as we set the weights outside the receptive field to 0
            # if l == self.n_layers - 2:
            #     grad_w += penalty * torch.sign(self.layers[l].weight)
            self.layers[l].weight.grad = grad_w
            if self.use_bias:
